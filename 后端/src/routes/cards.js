...
router.get('/', optionalAuth, async (req, res) => {
    try {
        const { page, pageSize, libraryId, chapterId } = req.query;
...
router.get('/hot', async (req, res) => {
    try {
        ...
        const cards = await CardModel.getHotCards(parseInt(limit) || 10);
...
router.post('/:id/like', async (req, res) => {
    try {
        ...
        const likeCount = await CardModel.incrementLikeCount(req.params.id);
...
router.post('/:id/unlike', async (req, res) => {
    try {
        ...
        const likeCount = await CardModel.decrementLikeCount(req.params.id);
